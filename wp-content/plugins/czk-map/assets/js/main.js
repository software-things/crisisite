jQuery(document).ready(function () {
    ...
    jQuery(document).on('click', '.marker-update', function () {
        ...
        markers.map((marker, i) => {
            if (i === index) {
                marker.address = jQuery(this).parent().find('input[name="address"]').val().replace("'", '"').trim();
                marker.phone = jQuery(this).parent().find('input[name="phone"]').val().replace("'", '"').trim();
                marker.www = jQuery(this).parent().find('input[name="www"]').val().replace("'", '"').trim();
                marker.email = jQuery(this).parent().find('input[name="email"]').val().replace("'", '"').trim();
                marker.content = jQuery(this).parent().find('textarea').val().replace("'", '"').trim();
            }
            return marker;
        });
        handleChange();
